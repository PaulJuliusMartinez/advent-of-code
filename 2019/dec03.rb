...
orig_strs = DATA.readlines.map(&:strip)
...
path1 = parse_path(orig_strs[0])
path2 = parse_path(orig_strs[1])

def process_path(path, h, n)
  x = 0
  y = 0
  ...
  path.each do |instruction|
    ...
    next_x += dist if dir == 'R'
    next_x -= dist if dir == 'L'
    next_y += dist if dir == 'U'
    next_y -= dist if dir == 'D'
    ...
    dist.times do |disp|
      ...
      h[[im_x, im_y]] ||= 1
      h[[im_x, im_y]] *= n if h[[im_x, im_y]] % n != 0
    end
...
points = {}
process_path(path1, points, 2)
process_path(path2, points, 3)

intersections = points.keys.select do |pt|
  points[pt] && points[pt] % 2 == 0 && points[pt] % 3 == 0
end

distances = (intersections - [[0, 0]]).map do |pt|
  pt[0].abs + pt[1].abs
end
...
def steps_to_intersection(path, intersections)
  x = 0
  y = 0
  ...
  path.each do |instruction|
    ...
    dist.times do |disp|
      ...
      if intersections.include?([im_x, im_y])
        steps[[im_x, im_y]] ||= total_steps - 1
      end
    end
...
path1_step_counts = steps_to_intersection(path1, intersections)
path2_step_counts = steps_to_intersection(path2, intersections)

result = intersections.min_by do |pt|
  path1_step_counts[pt] + path2_step_counts[pt]
end

puts path1_step_counts[result] + path2_step_counts[result]

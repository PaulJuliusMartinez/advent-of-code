...
def day11(part)
  ...
  panels.keys.each do |key|
    min_x = [min_x, key[0]].min
    max_x = [max_x, key[0]].max
    min_y = [min_y, key[1]].min
    max_y = [max_y, key[1]].max
  end

  if part == 2
    (min_y..max_y).each do |y|
      (min_x..max_x).each do |x|
        # Since we're going to min to max, this actually prints it up
        # print panels[[x, y]] == 'b' ? ' ' : '*'

        # Print it right-side up
        print panels[[x, max_y - (y - min_y)]] == 'b' ? ' ' : '*'
      end
      puts
    end
  end

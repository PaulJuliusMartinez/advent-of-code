...
loop do
  output = cpu.run([next_dir])[:value]

  dx, dy = input_to_dir[next_dir]
  if output == WALL
    env[[curr_pos[0] + dx, curr_pos[1] + dy]] = '#'
  elsif output == MOVED
  ...
  next_dir = [N, S, W, E].sample
  index += 1
  break
end

xs = env.keys.map(&:first)
ys = env.keys.map(&:last)
min_x = xs.min
max_x = xs.max
min_y = ys.min
max_y = ys.max
...
while to_process.any? do
  square = to_process.pop
  next if processed.include?(square)

  len = time_to_reach[square]

  x, y = square

  if map[[x + 1, y]] != '#'
    time_to_reach[[x + 1, y]] ||= len + 1
    to_process << [x + 1, y]
  end
  if map[[x - 1, y]] != '#'
    time_to_reach[[x - 1, y]] ||= len + 1
